...
func (h *HandlerCrypto) Add() error {
	if h.manager.block.IsLuksOpen() {
		logs.WithF(h.fields).Info("Already Open")
		return nil
	}
	...
	if err != nil {
		logs.WithEF(err, h.fields).Warn("Cannot check is luks name is already in use, continuing but it may fail")
	}
	if used {
		if h.manager.block.IsLuksUsed() {
			logs.WithF(h.fields).Warn("Luks already open for same block but not linked to device, trying to cleanup")

			h.cleanupRemovedBlockDevice(h.manager.block.GetUsableLabel())
		} else {
			return errs.WithF(h.fields.WithField("name", h.manager.block.GetUsableLabel()), "deviceMapper name is already used by another block")
		}
	}
	...
	if err := h.manager.block.LuksOpen(buffer); err != nil {
		return errs.WithEF(err, h.fields, "Failed to luks open")
	}
	return nil
}
...
func (h *HandlerCrypto) cleanupRemovedBlockDevice(label string) {
	...
	// manager
	manager := Manager{}
	manager.Init(fakeOpenedBlockDevice, &hdm.HDM)

	// mount handler
	handlerMount := handlers[handlerNameMount].new()
